...
BlueyPeer layers stable identity on top of the lifecycle protocol to solve cross-platform BLE identity instability.

### The problem

- **iOS** assigns `CBPeripheral.identifier` values that are stable within a single CoreBluetooth session but change across sessions (app restarts, device reboots).
- **Android** supports MAC randomization, so the platform-reported address for the same physical device varies over time.
- **Zombie advertisements** occur when a server app is force-killed but the OS continues advertising a stale peripheral.

### The solution

The control service hosts three characteristics:

```
control service (b1e70001-...)
├── heartbeat  (b1e70002-...)  write-with-response
├── interval   (b1e70003-...)  read (4-byte ms, little-endian)
└── serverId   (b1e70004-...)  read (16-byte UUID)
```

The `serverId` is a random v4 UUID generated by the server (or supplied by the app) that remains stable across restarts if persisted.

### Auto-upgrade

`bluey.connect(device)` automatically detects Bluey servers. After connecting, it discovers services. If the control service is present, the connection upgrades in place: it reads the `serverId`, starts the lifecycle heartbeat, and begins filtering the control service from the public services view. The `Connection.isBlueyServer` and `Connection.serverId` properties reflect the upgrade.

If the control service is absent at connect time (e.g., the server is still initializing), the connection starts as raw BLE. The connection can upgrade later via:

- **Service Changed events** — Android servers send Service Changed indications when their GATT database changes. iOS clients receive `peripheral(_:didModifyServices:)` and the connection auto-upgrades.
- **Manual service refresh** — calling `connection.services()` without cache re-discovers services and triggers the upgrade check. The example app's "refresh services" button does this.

### Discovery

- `bluey.discoverPeers()` scans, briefly connects to each candidate, reads `serverId`, disconnects. Returns `List<BlueyPeer>` deduplicated by id.
- `bluey.peer(serverId).connect()` performs a targeted scan, connects, verifies `serverId` matches, and starts the lifecycle heartbeat.
- `bluey.connect(device)` connects to any BLE device and auto-upgrades if it's a Bluey server.
