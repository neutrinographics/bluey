...
| Fast producer | High defensive cap (1024, mirroring `PendingNotificationQueue`) — a runaway-memory backstop, **not** the back-pressure mechanism. Back-pressure is complete-on-hand-off + the gate, which keep a serial consumer's depth at ~1, so the cap is effectively unreachable. `enqueue` returns `false` at cap and the caller fires its own `.failure`. |
