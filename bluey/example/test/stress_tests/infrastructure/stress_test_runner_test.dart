...
void main() {
  ...
  setUp(() {
    stressChar = FakeRemoteCharacteristic(uuid: UUID(StressProtocol.charUuid));
    conn = FakeConnection(
  ...
  group('StressTestRunner.runBurstWrite', () {
    test(
      'runs the configured count of writes and emits a final snapshot',
      () async {
    ...
    test(
      'reset write failure aborts with empty final snapshot, no crash',
      () async {
        stressChar.onWriteHook = (value, {required bool withResponse}) async {
    ...
    test(
      'emits incremental snapshots after each individual write completes',
      () async {
        stressChar.onWriteHook = (value, {required bool withResponse}) async {};
    ...
    test(
      'runBurstWrite stops publishing after subscription is cancelled',
      () async {
        stressChar.onWriteHook = (value, {required bool withResponse}) async {
  ...
  group('StressTestRunner.runMixedOps', () {
    ...
    test(
      'emits incremental snapshots after each individual op completes',
      () async {
        stressChar.onWriteHook = (value, {required bool withResponse}) async {};
  ...
  group('StressTestRunner.runSoak', () {
    test(
      'runs ops for the configured duration at the configured interval',
      () async {
        var writes = 0;
        stressChar.onWriteHook = (value, {required bool withResponse}) async {
          writes++;
  ...
  group('StressTestRunner.runTimeoutProbe', () {
    test(
      'sends DelayAck command sized past the timeout and counts the failure',
      () async {
            ...
            await runner
                .runTimeoutProbe(
                  const TimeoutProbeConfig(
                    delayPastTimeout: Duration(seconds: 2),
                  ),
                  conn,
                )
                .toList();
  ...
  group('StressTestRunner.runFailureInjection', () {
    test(
      'writes DropNext, then writeCount echoes — first echo throws timeout',
      () async {
        var echoCount = 0;
  ...
  group('StressTestRunner.runNotificationThroughput', () {
    ...
    test(
      'notification latencies are measured from burst start, not test start',
      () async {
        ...
        for (final latency in last.latencies) {
          expect(
            ...
            lessThan(500),
            reason:
                'latency should be measured from burst start, not test start',
          );
        }
      },
    );
    ...
    test(
      'drops notifications with stale burst-id (different from current)',
      () async {
        stressChar.onWriteHook = (value, {required bool withResponse}) async {
          if (value.isNotEmpty && value.first == 0x02) {
            Future<void>(() async {
