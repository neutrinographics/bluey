  ...
  group('AndroidServer', () {
    ...
    group('startAdvertising', () {
      test('maps config to DTO correctly', () async {
        ...
        final config = PlatformAdvertiseConfig(
          ...
          timeoutMs: 10000,
          mode: PlatformAdvertiseMode.balanced,
        );
        ...
        expect(captured.mode, equals(AdvertiseModeDto.balanced));
      });
      ...
      test('maps all advertise modes correctly', () async {
        ...
        for (final (platformMode, expectedDto) in [
          ...
          (PlatformAdvertiseMode.balanced, AdvertiseModeDto.balanced),
          (PlatformAdvertiseMode.lowLatency, AdvertiseModeDto.lowLatency),
        ]) {
          final config = PlatformAdvertiseConfig(
            serviceUuids: [],
            mode: platformMode,
    ...
    group('notifyCharacteristic', () {
      test('calls hostApi', () async {
        ...
        verify(() => mockHostApi.notifyCharacteristic('2A37', value)).called(1);
      });
    });
    ...
    group('notifyCharacteristicTo', () {
      test('calls hostApi', () async {
        ...
        verify(() =>
                mockHostApi.notifyCharacteristicTo('central-1', '2A37', value))
            .called(1);
      });
    });
    ...
    group('onReadRequest', () {
      test('emits to readRequests stream', () async {
        ...
        expect(request.characteristicUuid, equals('2A37'));
        expect(request.offset, equals(0));
      });
    });

    group('onWriteRequest', () {
      test('emits to writeRequests stream', () async {
